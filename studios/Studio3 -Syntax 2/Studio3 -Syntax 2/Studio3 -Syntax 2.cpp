...
int parser(std::vector<std::string>&vec,  int& chris);
int real_parser(std::vector<std::string>&vec, int& chris);
bool function(std::vector<std::string>&vec, int& chris);
bool arg(std::vector<std::string>&vec, int& chris);
...
int main(int argc, char * argv[])
{
	...
	if (file.is_open())
	{
		while (std::getline(file, s))
		{
			...
			Parse(s, string_vec);
			int chris = 0;
			int index = parser(string_vec, chris);
			if (index != -1){
				std::vector<std::string> new_vec(string_vec.begin(), string_vec.begin() + index);
				ChrisJJ.push_back(new_vec);
			}
			else {
				std::cout << "invalid line detected" << std::endl;
			}
...
int parser(std::vector<std::string>&vec, int& chris){
	if (vec.size() < 3){
		std::cout << " hered 1" << std::endl;
		return -1;
	}
	if ( CheckLabel(vec[chris] ) ){ 
		bool valid = true;
		valid = CheckLeftPa(vec[++chris]) && CheckRightPa(vec[++chris]);
		
		if (valid) return ++chris;
		else{ 
			std::cout << " hered 2" << std::endl;
			return -1; }
	}
	else{
		std::cout << " hered 3" << std::endl;
		return -1;
	}
...
bool arg(std::vector<std::string>&vec, int& chris){
	...
	else if (CheckInt(vec[chris + 1])){
		if (CheckInt(vec[chris])) {
