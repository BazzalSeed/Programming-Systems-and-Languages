...
int OpenIO(std::string input, std::string output)
{
	...
	//Testing the vector of valid token vectors
	/*
	for (auto i : valid_vectors){
		for (auto j : i){
			cout << j.token_value;
	...
	*/
	// Parsing again to create the AST
	for (auto token_vec : valid_vectors){
		
		s.AdoptVector(token_vec);
		// The while loop drives the parsing process
		vector<vector<Token>> valid_vectors;
		hornclause(s);
