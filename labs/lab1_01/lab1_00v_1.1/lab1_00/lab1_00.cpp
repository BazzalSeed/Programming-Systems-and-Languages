...
void _hornclause(vector<shared_ptr<Token> > & vec, shared_ptr<Hornclause> & n, int & index);
void _head(vector<shared_ptr<Token> > & vec, shared_ptr<Head> & n, int & index);
void _predicate(vector<shared_ptr<Token> > & vec, shared_ptr<Head> & n, int & index);
void _predicate(vector<shared_ptr<Token> > & vec, shared_ptr<Body> & n, int & index);
void _name(vector<shared_ptr<Token> > & vec, shared_ptr<Predicate> & n, int & index);
void _x_3(vector<shared_ptr<Token> > & vec, shared_ptr<Predicate> & n, int & index);
void _x_4(vector<shared_ptr<Token> > & vec, shared_ptr<Predicate> & n, int & index);
void _args(vector<shared_ptr<Token> > & vec, shared_ptr<Predicate> & n, int & index);
void _symbol(vector<shared_ptr<Token> > & vec, shared_ptr<Predicate> & n, int & index);
void _x_5(vector<shared_ptr<Token> > & vec, shared_ptr<Predicate> & n, int & index);
void _x_1(vector<shared_ptr<Token>> & vec, shared_ptr<Body> & n, int & index);
void _x_2(vector<shared_ptr<Token>> & vec, shared_ptr<Body> & n, int & index);
void _x_6(vector<shared_ptr<Token> > & vec, shared_ptr<Head> & n, int & index);
void _body(vector<shared_ptr<Token>> & vec, shared_ptr<Body> & n, int & index);
...
void _hornclause(vector<shared_ptr<Token>> & vec, shared_ptr<Hornclause> & n, int & index)
{
	...
	{
		auto head = make_shared<Head>("head");
		_head(vec, head, index);
		n->AdoptLHSChild(head);
		auto body = make_shared<Body>("body");
		_x_1(vec, body, index);
		n->AdoptRHSChild(body);
	}
}
...
void _x_2(vector<shared_ptr<Token>> & vec, shared_ptr<Body> & n, int & index)
{
	...
	{
		index += 1;
		auto sib_body = make_shared<Body>("body");
		_body(vec,sib_body,index);
		n->AdoptSibling(sib_body);
	}
	else
	{
		return;
...
void _x_6(vector<shared_ptr<Token> > & vec, shared_ptr<Head> & n, int & index)
{
	...
	{
		index += 1;
		auto sib_head = make_shared<Head>("head");
		_head(vec, sib_head, index);
		n->AdoptSibling(sib_head);
	}
	else if (vec[peek]->token_type == TOKENS::SEPARATOR)
	{
		return;
...
void _predicate(vector<shared_ptr<Token> > & vec, shared_ptr<Body> & n, int & index)
{
	...
	{
		auto pred = make_shared<Predicate>("predicate");
		_name(vec, pred, index);
		n->AdoptLHSChild(pred);
		_x_3(vec, pred, index);
	}
}
...
void _name(vector<shared_ptr<Token> > & vec, shared_ptr<Predicate> & n, int & index)
{
	...
	{
		index += 1;
		n->AdoptTerminal(make_shared<Terminals>("LABEL",*(vec[index])));
	}
}
...
int OpenIO(std::string input, std::string output)
{
	...
	while (s.CanRead())
	{
		shared_ptr<Token> t = make_shared<Token>(Token());
	...
	std::vector<shared_ptr<Hornclause>> tree_vec;
	for (auto vec : valid_vectors)
	{
		auto root = std::make_shared<Hornclause>("hornclause");
		try
		{
			int index = -1;
